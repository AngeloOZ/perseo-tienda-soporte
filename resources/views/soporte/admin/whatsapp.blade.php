                                ...
                                <div class="row">
                                    <div class="col-6">
                                        <div class="form-group">
                                            <label for="numerosWhats">Número de teléfono</label>
                                            <select class="form-control" id="numerosWhats">
                                                @foreach ($numeros as $numero)
                                                    <option value="{{ $numero->numero }}" selected>{{ $numero->numero }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="col-6 user-select-none" style="font-size: 17px;">
    ...
    <script>
        ...
        async function iniciar() {
            try {
                ...
                const {
                    data: result
                } = await axios.post("{{ route('whatsapp.iniciar') }}", body);
        ...
        async function obtenerQR() {
            try {
                ...
                const {
                    data: result
                } = await axios.post("{{ route('whatsapp.obtener.qr') }}", body);
        ...
        async function cerrarSesion() {
            try {
                ...
                const {
                    data: result
                } = await axios.post("{{ route('whatsapp.cerrar') }}", body);
        ...
        async function eliminarToken() {
            try {
                ...
                const {
                    data: result
                } = await axios.post("{{ route('whatsapp.eliminar_token') }}");
                await mostrarAlertas(result.message);
                location.reload();
            } catch (error) {
                console.log(error);
